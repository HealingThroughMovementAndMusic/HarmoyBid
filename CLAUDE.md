...
When a client signs via `/sign/:id`, `supabase/functions/sign-quote/index.ts` sends the business (`healingthroughmovementandmusic@gmail.com`, hardcoded there — kept in sync manually with `BUSINESS_PROFILE.email` in `src/lib/business/businessProfile.ts`, since Edge Functions can't import from `src/`) a notification email via the [Resend](https://resend.com) API, dispatched through `EdgeRuntime.waitUntil(...)` so it never blocks or fails the actual sign response. Requires the `RESEND_API_KEY` Supabase secret (same Dashboard → Secrets Management path as the Google key above; free tier is enough — no domain verification needed since delivery is to the account's own signup address). Missing the secret only logs an error and skips the send; it does not break signing.

`RESEND_API_KEY` **is now set**. A live test sign against `sign-quote` returned `200` with the full updated quote (auth + DB update path fully confirmed) — the email dispatch itself is fire-and-forget background work (`EdgeRuntime.waitUntil`), so its own success can only be confirmed by checking the inbox at `healingthroughmovementandmusic@gmail.com` directly, which wasn't independently verified.
